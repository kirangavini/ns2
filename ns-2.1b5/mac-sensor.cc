...
MacSensor::MacSensor() : Mac(), mhDefer_(this, 0.005), mhTx_(this), 
  mhRx_(this), code_(0), node_num_(0), ss_(0), CHheard_(0), myADVnum_(0)
{
...
MacSensor::send(Packet *p, Handler *h)
{
   ...
   */
  if(!is_idle() || netif_->csEnd(code) > Scheduler::instance().clock()) {
    /*
...
MacSensor::recv(Packet *p, Handler *h)
{
  ...
  if (new_code != code_) {
      /*
      printf("In MAC_IDLE %d: %d is not a code I am currently receiving 
